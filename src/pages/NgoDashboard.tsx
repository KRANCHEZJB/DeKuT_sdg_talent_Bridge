...
import { getProjects, createProject, getNgoProfile, createNgoProfile, getApplications, updateApplicationStatus } from '../api/api'
...
const NgoDashboard = () => {
  ...
  const loadData = async () => {
    setLoading(true)
    try {
      const [projRes, profileRes] = await Promise.allSettled([getProjects(), getNgoProfile()])
  ...
  const loadApplications = async (projectId: string) => {
    try {
      const res = await getApplications(projectId)
  ...
  const handleCreateProject = async () => {
    ...
    try {
      ...
      showToast('✅ Project created! Set it to "open" so students can apply.')
  ...
  const handleUpdateApplication = async (projectId: string, applicationId: string, status: string) => {
    try {
                ...
                <div style={{ animation:'fadeUp 0.4s ease' }}>
                  ...
                  <div className="stats-row" style={{ display:'grid', gridTemplateColumns:'repeat(4,1fr)', gap:'12px', marginBottom:'24px' }}>
                    {[
                      ...
                      { label:'In Progress', value:projects.filter(p=>p.project_status==='in_progress').length, color:'#FDB913' },
              ...
              {tab === 'create' && (
                <div style={{ animation:'fadeUp 0.4s ease', maxWidth:'640px' }}>
                  ...
                  <div style={{ background:'rgba(255,255,255,0.04)', border:'1px solid rgba(255,255,255,0.08)', borderRadius:'16px', padding:'28px', display:'flex', flexDirection:'column', gap:'16px' }}>
                    ...
                    <div>
                      <label style={{ color:'#94A3B8', fontSize:'11px', fontWeight:600, textTransform:'uppercase', letterSpacing:'0.5px', display:'block', marginBottom:'5px' }}>Skills Required <span style={{ color:'#475569', textTransform:'none', fontWeight:400 }}>(comma separated)</span></label>
                              ...
                              <div style={{ display:'flex', alignItems:'center', gap:'8px', flexWrap:'wrap' }}>
                                ...
                                <div style={{ display:'flex', gap:'5px' }}>
                                  {['shortlisted','selected','rejected'].map(s => (
                                    <button key={s} className="status-btn" onClick={() => handleUpdateApplication(selectedProject.id, app.application_id, s)}
                ...
                <div style={{ animation:'fadeUp 0.4s ease', maxWidth:'600px' }}>
                  ...
                  {profile && (
                    <div style={{ background:'rgba(255,255,255,0.03)', border:'1px solid rgba(255,255,255,0.07)', borderRadius:'16px', padding:'20px', marginBottom:'20px' }}>
                      <p style={{ fontSize:'12px', fontWeight:600, color:'#94A3B8', textTransform:'uppercase', letterSpacing:'0.5px', marginBottom:'14px' }}>Profile Preview — visible to students</p>
                      <div style={{ display:'flex', alignItems:'center', gap:'12px', marginBottom:'12px' }}>
                        <div style={{ width:'44px', height:'44px', borderRadius:'10px', background:'linear-gradient(135deg,#00A651,#0A6EBD)', display:'flex', alignItems:'center', justifyContent:'center', fontSize:'20px' }}>
                          {ORG_TYPES.find(t=>t.value===profile.organization_type)?.icon || '🏢'}
                        </div>
                        <div>
                          <p style={{ fontWeight:700, fontSize:'16px' }}>{profile.organization_name}</p>
                          <p style={{ color:'#94A3B8', fontSize:'12px' }}>{ORG_TYPES.find(t=>t.value===profile.organization_type)?.desc || profile.organization_type}</p>
                        </div>
                      </div>
                      <p style={{ color:'#94A3B8', fontSize:'13px', lineHeight:1.6 }}>{profile.mission_statement}</p>
                      <div style={{ display:'flex', gap:'16px', marginTop:'12px', flexWrap:'wrap' }}>
                        {profile.website && <a href={profile.website} target="_blank" rel="noreferrer" style={{ color:'#4ADE80', fontSize:'12px', textDecoration:'none' }}>🌐 Website</a>}
                        {profile.country && <span style={{ color:'#94A3B8', fontSize:'12px' }}>📍 {profile.country}</span>}
                        {profile.contact_phone && <span style={{ color:'#94A3B8', fontSize:'12px' }}>📞 {profile.contact_phone}</span>}
                      </div>
                    </div>
                  )}
