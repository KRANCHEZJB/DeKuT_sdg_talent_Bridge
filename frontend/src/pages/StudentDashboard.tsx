...
import { getProjects, applyToProject, getStudentProfile, createStudentProfile, getMyApplications } from '../api/api'
                    ...
                    <div className="projects-grid" style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill, minmax(320px,1fr))', gap:'16px' }}>
                      {filteredProjects.map((project, i) => (
                        <div key={project.id} className="dash-card" style={{ padding:'24px', animationDelay:`${i*0.05}s` }}>
                          ...
                          <button
                            className={`apply-btn ${hasApplied(project.id) ? 'applied' : ''}`}
                            onClick={() => {
                ...
                <div style={{ animation:'fadeUp 0.4s ease', maxWidth:'580px' }}>
                  ...
                  <div style={{ background:'rgba(255,255,255,0.04)', border:'1px solid rgba(255,255,255,0.08)', borderRadius:'16px', padding:'24px' }}>
                    ...
                    <div style={{ display:'flex', flexDirection:'column', gap:'14px' }}>
                      ...
                      <div>
                        <label style={{ color:'#94A3B8', fontSize:'11px', fontWeight:600, textTransform:'uppercase', letterSpacing:'0.5px', display:'block', marginBottom:'5px' }}>Profile Slug *</label>
                      ...
                      <div>
                        ...
                        <textarea className="profile-input" placeholder="Tell NGOs about yourself..." value={profileForm.bio} onChange={e => setProfileForm({...profileForm, bio: e.target.value})} rows={4} style={{ resize:'vertical' }} />
                      </div>
                      <div>
                        <label style={{ color:'#94A3B8', fontSize:'11px', fontWeight:600, textTransform:'uppercase', letterSpacing:'0.5px', display:'block', marginBottom:'5px' }}>Skills (comma separated)</label>
                        <input className="profile-input" placeholder="Python, Data Analysis, Research..." value={skillsInput} onChange={e => setSkillsInput(e.target.value)} />
                        {skillsInput && (
